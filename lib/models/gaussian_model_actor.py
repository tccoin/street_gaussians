...
class GaussianModelActor(GaussianModel):
    ...
    def densify_and_prune(self, max_grad, min_opacity, prune_big_points):
        if self.get_xyz.shape[0] == 0:
            ...
            self.xyz_gradient_accum = torch.zeros((0, 2), device="cuda")
            self.denom = torch.zeros((0, 1), device="cuda")
            self.max_radii2D = torch.zeros((0,), device="cuda")
        ...
        if prune_big_points or cfg.optim.get('actor_box_prune', False):
            box_scale = float(cfg.optim.get('actor_box_prune_scale', 1.0))
            min_xyz = self.min_xyz * box_scale
            max_xyz = self.max_xyz * box_scale
            points_outside_box = torch.logical_or(
                torch.any(self.get_xyz < min_xyz, dim=-1),
                torch.any(self.get_xyz > max_xyz, dim=-1),
            )

            repeat_num = int(cfg.optim.get('actor_box_prune_sample_count', 2))
            if repeat_num > 0:
                # Sample each Gaussian footprint so large splats crossing the box edge are removed.
                ...
                rots = quaternion_to_matrix(self.get_rotation)
                rots = rots[:, None, :, :].expand(-1, repeat_num, -1, -1)
                origins = self.get_xyz[:, None, :].expand(-1, repeat_num, -1)
                samples_xyz = torch.matmul(rots, samples.unsqueeze(-1)).squeeze(-1) + origins
                samples_inside_box = torch.logical_and(
                    torch.all((samples_xyz >= min_xyz).reshape(num_gaussians, repeat_num * 3), dim=-1),
                    torch.all((samples_xyz <= max_xyz).reshape(num_gaussians, repeat_num * 3), dim=-1),
                )
                points_outside_box = torch.logical_or(points_outside_box, torch.logical_not(samples_inside_box))

            prune_mask = torch.logical_or(prune_mask, points_outside_box)
            self.scalar_dict['points_outside_box'] = points_outside_box.sum().item()
            self.scalar_dict['actor_box_prune_scale'] = box_scale
